...
export const IMPECCABLE_SITE_WARMUP_CONFIG = Object.freeze({
  domains: Object.freeze(['impeccable.472158246.workers.dev', 'impeccable.hagicode.com']),
  maxAttempts: 4,
  retryDelayMs: 3000,
  timeoutMs: 10000,
});
...
export function normalizeWarmupConfig(config = IMPECCABLE_SITE_WARMUP_CONFIG) {
...
export async function warmDomain(domain, { config = IMPECCABLE_SITE_WARMUP_CONFIG, fetchImpl = globalThis.fetch, wait = sleep, logger = console } = {}) {
...
export async function runWarmup({ config = IMPECCABLE_SITE_WARMUP_CONFIG, fetchImpl = globalThis.fetch, wait = sleep, logger = console } = {}) {
